...
Exopilot is structured around all five SOLID design principles across both the Go API and the Next.js frontend layers.

```mermaid
mindmap
  root((SOLID))
    S["S — Single Responsibility"]
      "`**firebase.go** handles only Firebase/Firestore init & persistence`"
      "`**security.ts** handles only OWASP input sanitization & rate limiting`"
      "`Each React component owns exactly one UI domain`"
    O["O — Open / Closed"]
      "`**apiFetch** is open for extension via custom RequestInit options`"
      "`Offline fallback handler is closed to modification — new routes add to the switch, never rewrite core`"
    L["L — Liskov Substitution"]
      "`Firestore persistence and in-memory store are interchangeable — Go API uses same data shapes regardless of backend`"
      "`AuthContext exposes a stable interface; underlying Firebase SDK can be swapped`"
    I["I — Interface Segregation"]
      "`**Profile** interface is split from **BuyerInquiry**, **ShipmentTracker**, **Supplier** — consumers import only what they need`"
      "`API exports are tree-shakeable named functions, not a single monolithic client`"
    D["D — Dependency Inversion"]
      "`Components depend on **useAuth()** context, not on Firebase SDK directly`"
      "`Go handlers depend on abstract Firestore CollectionRef, not on concrete HTTP transport`"
```
...
The Firestore data model uses UID-scoped document collections with the following relationships:
...
erDiagram
    ...
    BUYER_INQUIRY {
        string id PK
        string user_id FK "→ PROFILE.id"
        string buyer_name
        string country
        string product
        string quantity
        string status "New | Negotiating | Closed"
        string price_offer
        string notes
        string negotiation_notes
        timestamp created_at
        timestamp updated_at
    }
    ...
    SHIPMENT_TRACKER {
        ...
        string commodity
        string status "Customs | On Vessel | Arrived"
        timestamp etd
        timestamp eta
        timestamp created_at
        timestamp updated_at
    ...
    NEGOTIATION_NOTE {
        ...
        string buyer_name
        string date
        string category "Risk | Payment | Preference"
        string content
        string[] tags
    }
    ...
    BUYER_CRM {
        ...
        int trust_level "1–5"
        string preferences
        string[] preferred_products
        string communication_notes
        object[] deal_history
    }

    PROFILE ||--o{ BUYER_INQUIRY : "owns"
    PROFILE ||--o{ SHIPMENT_TRACKER : "owns"
    PROFILE ||--o{ SUPPLIER : "owns"
    PROFILE ||--o{ NEGOTIATION_NOTE : "owns"
    PROFILE ||--o{ BUYER_CRM : "owns"
    BUYER_INQUIRY }o--|| BUYER_CRM : "references buyer"
    SHIPMENT_TRACKER }o--|| BUYER_CRM : "linked to buyer"
```
...
The following class diagram covers the key TypeScript interfaces, React context classes, and the Go API handler model:
...
classDiagram
    ...
    class AuthContextType {
        ...
        +boolean loading
        +boolean isAdmin
        +signup(email, password, company, legalType) Promise~void~
        +login(email, password) Promise~void~
        +logout() Promise~void~
        +updateProfilePhoto(url) Promise~void~
        +refreshProfile() Promise~Profile~
    }

    class SecurityModule {
        +sanitizeInput(value) string
        +sanitizeTextField(value, maxLength) string
    ...
    class ApiClient {
        ...
        -handleLocalRequest(path, options) Promise~any~
        +fetchDashboard() Promise~any~
        +createInquiry(body) Promise~any~
        +updateInquiry(id, body) Promise~any~
        +deleteInquiry(id) Promise~any~
        +createShipment(body) Promise~any~
        +toggleShipmentDocument(id, docType) Promise~any~
        +createSupplier(body) Promise~any~
        +updateSupplier(id, body) Promise~any~
        +deleteSupplier(id) Promise~any~
        +createNote(body) Promise~any~
        +deleteNote(id) Promise~any~
        +updateBuyerCRM(id, body) Promise~any~
        +generateDocument(body) Promise~any~
    }
    ...
    class GoApiHandler {
        ...
        +HandleShipment(w, r)
        +HandleSupplier(w, r)
        +HandleNegotiationNote(w, r)
        +HandleGenerateDocument(w, r)
    ...
    class FirebaseModule {
        <<firebase.go>>
        +InitFirebase(credPath, projectId)
        +GetFirestoreClient() FirestoreClient
        +IsInitialized() bool
        -findCredentialsFile() string
    }

    AuthContextType --> Profile : "holds"
    AuthContextType --> SecurityModule : "uses for validation"
    ApiClient --> GoApiHandler : "HTTP REST calls"
    GoApiHandler --> FirebaseModule : "initializes & uses"
    GoApiHandler --> BuyerInquiry : "reads/writes"
    GoApiHandler --> ShipmentTracker : "reads/writes"
    GoApiHandler --> Supplier : "reads/writes"
    Profile "1" --> "0..*" BuyerInquiry : "user_id"
    Profile "1" --> "0..*" ShipmentTracker : "user_id"
    Profile "1" --> "0..*" Supplier : "user_id"
```
